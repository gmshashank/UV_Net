...
class Segmentation(pl.LightningModule):
    ...
    def __init__(self, num_classes,batch_size, crv_in_channels=6):
        """
        Args:
        ...
        self.train_iou = torchmetrics.JaccardIndex(task="multiclass",
            num_classes=num_classes, compute_on_step=False
        )
        self.val_iou = torchmetrics.JaccardIndex(task="multiclass",num_classes=num_classes, compute_on_step=False)
        self.test_iou = torchmetrics.JaccardIndex(task="multiclass",num_classes=num_classes, compute_on_step=False)
    ...
    def training_step(self, batch, batch_idx):
        ...
        logits = self.model(inputs)
        loss = F.cross_entropy(logits, labels, reduction="mean")
        self.log("train_loss", loss, on_step=False, on_epoch=True, sync_dist=True,batch_size=self.batch_size)
        preds = F.softmax(logits, dim=-1)
